...
export type LegacyEstimationRow = {
  id: string;
  created_at: string | null;
  updated_at: string | null;
  name: string | null;
...
export type LegacySource = "contact" | "estimation";
export type MatchCategory = "MATCH CERTAIN" | "MATCH PROBABLE" | "AMBIGU" | "AUCUN MATCH";
export type PlannedAction = "CREATE_OR_REUSE_CONTACT" | "CREATE_CONTACT_WITH_LEAD" | "MANUAL_REVIEW";
...
export type LegacyCrmReviewSummary = {
  contactsRead: number;
  estimationsRead: number;
  ...
  simulatedCommunications: number;
};

export type LegacyCrmReviewReport = {
  success: true;
  mode: "dry-run";
  generatedAt: string;
  writesPerformed: false;
  summary: LegacyCrmReviewSummary;
  ambiguousCases: SimulatedPayload[];
  probableMatches: SimulatedPayload[];
  unmatched: SimulatedPayload[];
  simulatedPayloads: SimulatedPayload[];
};

export function normalizeLegacyText(value: string | null | undefined) {
  return (
    value
      ?.normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "")
...
export function normalizeLegacyEmail(value: string | null | undefined) {
  const email = value?.trim().toLowerCase();
  if (!email || !email.includes("@") || !email.includes(".")) return null;
  return email;
}

export function normalizeLegacyFrenchPhone(value: string | null | undefined) {
...
function toLegacyStatus(status: string | null | undefined) {
  ...
  if (["mandate_signed", "mandat signe"].includes(normalizedStatus)) return "MANDATE_SIGNED";
  if (["lost", "perdu"].includes(normalizedStatus)) return "LOST";
  return "NEW";
}

export function contactToLegacySubmission(row: LegacyContactRow): LegacySubmission {
  ...
  return {
    ...
    name,
    normalizedName: normalizeLegacyText(name),
    email: row.email?.trim() || null,
    normalizedEmail: normalizeLegacyEmail(row.email),
    phone: row.phone?.trim() || null,
    normalizedPhone: normalizeLegacyFrenchPhone(row.phone),
    city: row.city?.trim() || null,
    normalizedCity: normalizeLegacyText(row.city) || null,
...
export function estimationToLegacySubmission(row: LegacyEstimationRow): LegacySubmission {
...
export function buildLegacyPayloads(submissions: LegacySubmission[]) {
...
export function buildLegacyReviewSummary(
  payloads: SimulatedPayload[],
  contactCount: number,
...
export function buildLegacyCrmReviewReport(
  contacts: LegacyContactRow[],
  estimations: LegacyEstimationRow[],
  generatedAt = new Date().toISOString()
): LegacyCrmReviewReport {
  const submissions = [
    ...contacts.map(contactToLegacySubmission),
    ...estimations.map(estimationToLegacySubmission),
  ];
  const payloads = buildLegacyPayloads(submissions);
  ...
  return {
    ...
    generatedAt,
    writesPerformed: false,
    summary: buildLegacyReviewSummary(payloads, contacts.length, estimations.length),
